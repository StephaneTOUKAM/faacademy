...
class Course_Widget extends \WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		if ( ! empty($a['testage'])){
			$testage = (array) explode(',', $a['testage']);

			$a['tax_query'] = array(
				array(
					'taxonomy' => 'course-testage',
					'field'    => 'term_id',
					'terms'    => $testage,
					'operator' => 'IN',
				),
		...
		if ( ! empty($a['subject'])){
			$subject = (array) explode(',', $a['subject']);
	...
	public function form( $instance ) {
		...
		$testage = ! empty( $instance['testage'] ) ? $instance['testage'] : '';
