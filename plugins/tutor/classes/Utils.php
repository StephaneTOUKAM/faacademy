...
class Utils {
	...
	public function enrolled_courses_by_user( $user_id = 0, $post_status='publish' ) {
		global $wpdb;
	...
	public function tutor_dashboard_pages() {
		$nav_items = apply_filters( 'tutor_dashboard/nav_items', array(
			...
			'rehearsal-courses'        => array( 'title' => __( 'Private tutoring', 'tutor') ),
			'exam-preparation-courses'        => array( 'title' => __( 'Exam preparation courses', 'tutor') ),
			'ebooks'        => array( 'title' => __( 'E-Book', 'tutor') ),
			'instructors'        => array( 'title' => __( 'Instructors', 'tutor') ),
			'notifications'  => __( 'Notifications', 'tutor' ),	
		));
		...
		$instructor_nav_items = apply_filters( 'tutor_dashboard/instructor_nav_items', array(
			...
			'coupons'           => array( 'title' => __( 'My Discount coupons', 'tutor'), 'auth_cap' => tutor()->instructor_role ),
	...
	public function desactivate_rehearsal_ebook_product() {
		global $wpdb;
		...
		$rehearsal_ebook = new \WP_Query( $args );

		if ( $rehearsal_ebook->have_posts() ){
			foreach ( $rehearsal_ebook->posts as $post ){
				if ( get_post_meta($post->ID, "tutor_rehearsal_date_limite", true) && get_post_meta($post->ID, "tutor_rehearsal_date_limite", true) < date("Y-m-d") ){
					$modification = $wpdb->update(
						$wpdb->posts,
						array( 
							'post_status' => "inherit"
						),
		...
		return "Rehearsals E-Book desactivated successfully !";
	}
	...
	public function get_total_courses() {
		...
		$count = $wpdb->get_var( $wpdb->prepare(
			...
			FROM 	{$wpdb->posts} 
			WHERE 	{$wpdb->posts}.post_type = %s AND {$wpdb->posts}.post_status = 'publish'", 'Courses'
		) );
	...
	public function course_levels( $level = null ) {
		$levels_all = $this->get_course_level_term();
		$level_array = [];
		$level_array["all"] = __( 'All', 'edumall' );
		foreach ($levels_all as $key => $value) {
			$level_array[$value->slug] = $value->name;
		}
		$levels = apply_filters( 'tutor_course_level', $level_array);
	...
	public function course_levels_new() {
		$levels_all = $this->get_course_level_term();
		$level_array = [];
		$level_array["all"] = __( 'All', 'edumall' );
		foreach ($levels_all as $key => $value) {
			$level_array["$value->term_id"] = $value->name;
		}
		$levels = apply_filters( 'tutor_course_level', $level_array);
		return $levels;
	}
	...
	public function user_profile_permalinks() {
		$permalinks = array(
			'courses_taken'     => __( 'Created Courses', 'tutor' ),
		);
	...
	public function get_course_topics( $parent = 0 ) {
		$args = apply_filters( 'tutor_get_course_topics_args', array(
		...
		$children = array();
		foreach ( $terms as $term ) {
			$term->children = $this->get_course_topics( $term->term_id );
			$children[ $term->term_id ] = $term;
		}
	...
	public function get_course_subjects( $parent = 0 ) {
		$args = apply_filters( 'tutor_get_course_subjects_args', array(
		...
		$children = array();
		foreach ( $terms as $term ) {
			$term->children = $this->get_course_subjects( $term->term_id );
			$children[ $term->term_id ] = $term;
		}

		return $children;
	}
	public function get_course_levels( $parent = 0 ) {
		$args = apply_filters( 'tutor_get_course_levels_args', array(
		...
		$children = array();
		foreach ( $terms as $term ) {
			$term->children = $this->get_course_levels( $term->term_id );
			$children[ $term->term_id ] = $term;
		}
	...
	public function get_course_languages( $parent = 0 ) {
		$args = apply_filters( 'tutor_get_course_languages_args', array(
		...
		$children = array();
		foreach ( $terms as $term ) {
			$term->children = $this->get_course_languages( $term->term_id );
			$children[ $term->term_id ] = $term;
		}
	...
	public function get_course_tags_term( $parent_id = 0 ) {
		$args = apply_filters( 'tutor_get_course_tags_terms_args', array(
			'taxonomy'   => 'course-tag',
			'parent'     => $parent_id,
			'hide_empty' => false,
		));
	...
	public function get_course_subjects_term( $parent_id = 0 ) {
		$args = apply_filters( 'tutor_get_course_subjects_terms_args', array(
			'taxonomy'   => 'course-subject',
			'parent'     => $parent_id,
			'hide_empty' => false,
		));
	...
	public function get_course_topics_term( $parent_id = 0 ) {
		$args = apply_filters( 'tutor_get_course_topics_terms_args', array(
			'taxonomy'   => 'course-topic',
			'parent'     => $parent_id,
			'hide_empty' => false,
		));
	...
	public function get_course_level_term( $parent_id = 0 ) {
		$args = apply_filters( 'tutor_get_course_levels_terms_args', array(
			'taxonomy'   => 'course_level',
			'parent'     => $parent_id,
			'hide_empty' => false,
		));
	...
	*/
	...
	{
		return $this->has_user_role(array('administrator', 'editor', 'tutor-instructor')) || $this->is_instructor_of_this_course($user_id, $course_id);
	}
}