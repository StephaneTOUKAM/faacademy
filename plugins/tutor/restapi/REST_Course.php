...
class REST_Course  {
	...
	private $course_sub_tax="course-subject";

	private $course_top_tax="course-topic";

	private $course_lev_tax="course_level";
	...
	*return course info
	*pagination enable
	*category,tags,subject,topic,level terms included
	*/
	public function course(WP_REST_Request $request) {
		...
		if(count($query->posts)>0) {
			...
			foreach($query->posts as $post) {
				...
				$subject = wp_get_post_terms($post->ID,$this->course_sub_tax);

				$topic = wp_get_post_terms($post->ID,$this->course_top_tax);

				$level = wp_get_post_terms($post->ID,$this->course_lev_tax);
				...
				$post->course_subject = $subject;

				$post->course_topic = $topic;

				$post->course_level = $level;
	...
	*/
	public function course_by_terms(WP_REST_Request $request) {
		...
		$subjects = sanitize_term( $request['subjects'], $this->course_sub_tax, $context = 'db' );

		$topics = sanitize_term( $request['topics'], $this->course_top_tax, $context = 'db' );

		$levels = sanitize_term( $request['levels'], $this->course_lev_tax, $context = 'db' );
		...
		$args = array(
		    ...
		    'tax_query' => array(
				...
				array(
		            'taxonomy' => $this->course_sub_tax,
		            'field'    => 'name',
		            'terms'    => $subjects
		            
		        ),
				array(
		            'taxonomy' => $this->course_top_tax,
		            'field'    => 'name',
		            'terms'    => $topics
		            
		        ),
				array(
		            'taxonomy' => $this->course_lev_tax,
		            'field'    => 'name',
		            'terms'    => $levels
	...
	*/
	public function validate_terms(array $post) {
		...
		if (!is_array($topics)) {
			array_push($error,__('Topics field is not an array','tutor'));
		}

		if (!is_array($subjects)) {
			array_push($error,__('subjects field is not an array','tutor'));
		}
		if (!is_array($levels)) {
			array_push($error,__('levels field is not an array','tutor'));
		}
