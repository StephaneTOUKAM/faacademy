...
if ( ! function_exists('tutor_course_tags_dropdown')){
	function tutor_course_tags_dropdown($post_ID = 0, $args = array()){
		...
		$output = '';
		$output .= "<select name='{$name}' {$multiple_select} class='{$classes}' data-placeholder='". __('Search Course tag. ex. Design, Development, Business', 'tutor') ."'>";
...
if ( ! function_exists('tutor_course_subjects_dropdown')){
	function tutor_course_subjects_dropdown($post_ID = 0, $args = array()){

		$default = array(
			'classes'  => '',
			'name'  => 'tax_input[course-subject]',
			'multiple' => true,
		);

		$args = apply_filters('tutor_course_subjects_dropdown_args', array_merge($default, $args));
		...
		$output = '';
		$output .= "<select name='{$name}' {$multiple_select} class='{$classes}' data-placeholder='". __('Search Course subject. ex. Design, Development, Business', 'tutor') ."'>";
		$output .= "<option value=''>". __('Select a subject', 'tutor') ."</option>";
		$output .= _generate_subjects_dropdown_option($post_ID, $subjects, $args);
		$output .= "</select>";

		return $output;
	}
}
if ( ! function_exists('tutor_course_topics_dropdown')){
	function tutor_course_topics_dropdown($post_ID = 0, $args = array()){

		$default = array(
			'classes'  => '',
			'name'  => 'tax_input[course-topic]',
			'multiple' => true,
		);

		$args = apply_filters('tutor_course_topics_dropdown_args', array_merge($default, $args));
		...
		$topics = tutor_utils()->get_course_topics();

		$output = '';
		$output .= "<select name='{$name}' {$multiple_select} class='{$classes}' data-placeholder='". __('Search Course topic. ex. Design, Development, Business', 'tutor') ."'>";
		$output .= "<option value=''>". __('Select a topic', 'tutor') ."</option>";
		$output .= _generate_topics_dropdown_option($post_ID, $topics, $args);
		$output .= "</select>";

		return $output;
	}
}
if ( ! function_exists('tutor_course_levels_dropdown')){
	function tutor_course_levels_dropdown($post_ID = 0, $args = array()){

		$default = array(
			'classes'  => '',
			'name'  => 'tax_input[course_level]',
			'multiple' => true,
		);

		$args = apply_filters('tutor_course_levels_dropdown_args', array_merge($default, $args));
		...
		$output = '';
		$output .= "<select name='{$name}' {$multiple_select} class='{$classes}' data-placeholder='". __('Search Course level. ex. Basic, All level', 'tutor') ."'>";
		$output .= "<option value=''>". __('Select a topic', 'tutor') ."</option>";
		$output .= _generate_levels_dropdown_option($post_ID, $levels, $args);
		$output .= "</select>";
...
if ( ! function_exists('_generate_tags_dropdown_option')){
	function _generate_tags_dropdown_option($post_ID = 0, $tags, $args = array(), $depth = 0){
		$output = '';
		...
		if (tutor_utils()->count($tags)) {
			foreach ( $tags as $tag_id => $tag ) {
				...
				$output .= "<option value='{$tag->term_id}' ".selected($has_in_term, true, false)." >   {$depth_seperator} {$tag->name}</option> ";
...
if ( ! function_exists('_generate_subjects_dropdown_option')){
	function _generate_subjects_dropdown_option($post_ID = 0, $subjects, $args = array(), $depth = 0){
		$output = '';

		if (tutor_utils()->count($subjects)) {
			foreach ( $subjects as $subject_id => $subject ) {
				if ( ! $subject->parent){
					$depth = 0;
				}

				$childrens = tutor_utils()->array_get( 'children', $subject );
				$has_in_term = has_term( $subject->term_id, 'course-subject', $post_ID );
				...
				$output .= "<option value='{$subject->term_id}' ".selected($has_in_term, true, false)." >   {$depth_seperator} {$subject->name}</option> ";

				if ( tutor_utils()->count( $childrens ) ) {
					$depth++;
					$output .= _generate_subjects_dropdown_option($post_ID,$childrens, $args, $depth);
				}
			}
		}
		return $output;
...
if ( ! function_exists('_generate_topics_dropdown_option')){
	function _generate_topics_dropdown_option($post_ID = 0, $topics, $args = array(), $depth = 0){
		$output = '';

		if (tutor_utils()->count($topics)) {
			foreach ( $topics as $topic_id => $topic ) {
				if ( ! $topic->parent){
					$depth = 0;
				}

				$childrens = tutor_utils()->array_get( 'children', $topic );
				$has_in_term = has_term( $topic->term_id, 'course-topic', $post_ID );
				...
				$output .= "<option value='{$topic->term_id}' ".selected($has_in_term, true, false)." >   {$depth_seperator} {$topic->name}</option> ";

				if ( tutor_utils()->count( $childrens ) ) {
					$depth++;
					$output .= _generate_topics_dropdown_option($post_ID,$childrens, $args, $depth);
				}
			}
		}
		return $output;
...
if ( ! function_exists('_generate_levels_dropdown_option')){
	function _generate_levels_dropdown_option($post_ID = 0, $levels, $args = array(), $depth = 0){
		$output = '';

		if (tutor_utils()->count($levels)) {
			foreach ( $levels as $level_id => $level ) {
				if ( ! $level->parent){
					$depth = 0;
				}

				$childrens = tutor_utils()->array_get( 'children', $level );
				$has_in_term = has_term( $level->term_id, 'course_level', $post_ID );
				...
				$output .= "<option value='{$level->term_id}' ".selected($has_in_term, true, false)." >   {$depth_seperator} {$level->name}</option> ";

				if ( tutor_utils()->count( $childrens ) ) {
					$depth++;
					$output .= _generate_levels_dropdown_option($post_ID,$childrens, $args, $depth);
				}
			}
		}
		return $output;
