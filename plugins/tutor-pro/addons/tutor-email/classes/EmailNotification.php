...
class EmailNotification {
	...
	public function __construct() {
		...
		add_action('tutor_rehearsal_notify_everyone', array($this, 'tutor_rehearsal_notify_everyone'), 10, 3);
		add_action('tutor_rehearsal_notify_admin_deletion_rehearsal', array($this, 'tutor_rehearsal_notify_admin_deletion_rehearsal'), 10, 3);
	...
	public function send($to, $subject, $message, $headers, $attachments = array(), $force_enqueue=false) {
    	...
    	//$this->enqueue_email("cindy@smartcodegroup.com", "Ton test remarche mec", $message, $headers, $attachments, $force_enqueue);
    // 	var_dump("Ton test remarche mec");
    // 	var_dump($to);
    	if (is_array($to)) {
    		foreach ($to as $key => $emailTo) {
    			$user = get_user_by( 'email', $emailTo );
    			$userId = $user->ID;
    			$my_post = array(
    				'post_title'    => $userId,
    				'post_name'    => $subject,
    				...
    				'post_status'   => 'publish',
    				'post_author'   => 1,
    				'post_type'    => 'notifdashcus'
    			);
    			
    			// Insert the post into the database
    			wp_insert_post( $my_post );
    			$this->enqueue_email($emailTo, $subject, $message, $headers, $attachments, $force_enqueue);
    		}
    	}else{
	...
	/**
	 * After save new rehearsal or ebook
	 *
	 * @since 1.6.9
	 */
	public function tutor_rehearsal_notify_everyone($announcement_id, $announcement, $action_type) {
		$site_url = get_bloginfo( 'url' );
		$site_name = get_bloginfo( 'name' );
		$course_name = get_the_title($announcement->ID);
		$course_url = get_the_permalink($announcement->ID);
		$announcement_content = get_post_meta($announcement_id, "tutor_rehearsal_description", true);
		$student_emails = [get_option( 'admin_email' ), wp_get_current_user()->user_email];
		$type_alert = '';
		if ($action_type == 'create') {
			if (get_post_meta($announcement_id, "tutor_rehearsal_type_data", true) == "E-Book") {
				$subject = __('New E-Book added', 'tutor-pro');
				$type_alert = "E-Book";
			} else {
				if (get_post_meta($announcement_id, "tutor_rehearsal_service_type", true) == "Rehearsal Course") {
					$subject = __('New Rehearsal Course added', 'tutor-pro');
					$type_alert = "Rehearsal Course";
				}else{
					$subject = __('New Exam Preparation Course added', 'tutor-pro');
					$type_alert = "Exam Preparation Course";
				}  
			}
			$template = 'to_admin_instructor_rehearsal_ebook_created';
			...
			$this->send($student_emails[1], $subject, $message, $headers);
			$this->send($student_emails[0], $subject, $message, $headers);
		}

	}

	/**
	 * After delete new rehearsal or ebook
	 *
	 * @since 1.6.9
	 */
	public function tutor_rehearsal_notify_admin_deletion_rehearsal($name, $action_type) {
		$course_name = $name;
		$student_emails = [get_option( 'admin_email' )];
		
		if ($action_type == "E-Book") {
			$subject = __('E-Book deleted', 'tutor-pro');
			$type_alert = "E-Book";
		} else {
			$subject = __('Rehearsal Course deleted', 'tutor-pro');
			$type_alert = "Rehearsal Course";
		}
		...
		$template = 'to_admin_rehearsal_ebook_deleted';
