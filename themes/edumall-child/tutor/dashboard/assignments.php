...
/**
 * WPML: get_assignments_by_instructor() ignores the current language, so
 * filter the assignments down to it here based on their linked course.
 */
$assignments->results = edumall_wpml_filter_items_by_course_id( $assignments->results, function ( $item ) {
	return tutor_utils()->get_course_id_by_assignment( $item->ID );
} );
$assignments->count   = is_array( $assignments->results ) ? count( $assignments->results ) : 0;
?>
