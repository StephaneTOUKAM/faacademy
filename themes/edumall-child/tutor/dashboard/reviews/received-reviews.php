...
/**
 * WPML: get_reviews_by_instructor() ignores the current language, so filter
 * the reviews down to it here based on their linked course.
 */
$reviews->results = edumall_wpml_filter_items_by_course_id( $reviews->results, function ( $review ) {
	return $review->comment_post_ID;
} );
$reviews->count   = is_array( $reviews->results ) ? count( $reviews->results ) : 0;
?>
