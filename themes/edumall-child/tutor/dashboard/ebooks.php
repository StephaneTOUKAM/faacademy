...
<div class="new-announcement-button" style="margin-bottom: 2rem;display: flex;flex-wrap: wrap;">
	<h3><?php esc_html_e( 'E-Books', 'edumall' ); ?></h3>
</div>
<?php
	if ( in_array( 'tutor_instructor', (array) $user->roles ) ) {
?>
			...
			<div class="withdraw-page-current-balance new-announcement-wrap" style="flex-wrap: wrap;">
				<div class="balance-info new-announcement-content" style="flex: 0.9;margin-bottom: 10px;">
					...
					<div>
						...
						<small><?php esc_html_e( 'Create E-Books', 'edumall' ); ?></small>
						<p>
							<strong>
				...
				<div class="new-announcement-button" style="width: auto;flex: 1;min-width: min(400px,100%);max-width: 100%;">
					<button type="button" class="tutor-btn tutor-announcement-add-new" style="width: auto;max-width: unset;/* flex-basis: auto; */flex: auto;height: auto;line-height: 26px;min-height: 54px;/* margin-top: 20px; */">
						<?php esc_html_e( 'Add New E-Book', 'edumall' ); ?>
					</button>
				</div>
			</div>
		</div>
...
<div class="tutor-announcement-table-wrap dashboard-table-wrapper dashboard-table-responsive">
	<div class="dashboard-table-container">
		<table class="dashboard-table ebook-table">
			<thead>
				<tr>
					...
					<?php
						if ( in_array( 'tutor_instructor', (array) $user->roles ) ) {
					?>
						<th style="width:24%"><?php esc_html_e( 'Files', 'edumall' ); ?></th>
						<th style="width:24%"><?php esc_html_e( 'Status', 'edumall' ); ?></th>
				...
				<?php if ( $rehearsal_courses->have_posts() && $total > 0 && $total_ebook > 0 ) : ?>
					<?php foreach ( $rehearsal_courses->posts as $post ) : ?>
						<?php
							if ( in_array( 'tutor_instructor', (array) $user->roles ) ) {
						?>
							<?php if ( $post->post_excerpt == "rehearsal-course-".get_current_user_id() && get_post_meta($post->ID, "tutor_rehearsal_type_data", true) == "E-Book" ) : ?>
								<tr id="tutor-rehearsal-tr-<?php echo $post->ID; ?>">
									<td class="tutor-announcement-date"><a href="<?php echo $post->post_status == "draft" ? "#" : $post->guid; ?>" style="color:#0071dc;"><?php echo $post->post_title; ?></a></td>
									<td class="tutor-announcement-date">
										<?php 
											foreach (json_decode(get_post_meta($post->ID, "tutor_rehearsal_file", true)) as $key => $value) {
												$key_new = $key + 1;
												echo '<a href="'.$value.'" target="_blank">View file '.$key_new.' | </a>';
											} 
										?>
									</td>
									<?php
											...
											<div class="tutor-announcement-buttons">
												<li class="edit-li">
													<a type="button" class="tutor-btn bordered-btn tutor-rehearsal-details edit-btn" title="Edit" 
														...
														rehearsal-file='<?php echo get_post_meta($post->ID, "tutor_rehearsal_file", true); ?>'
...
<?php
include 'ebooks/create.php';
include 'ebooks/update.php';
