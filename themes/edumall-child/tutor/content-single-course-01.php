...
if(!tutor_utils()->is_course_purchasable(get_the_ID()) && get_post_meta(get_the_ID(), '_tutor_is_public_course', true) != "yes" && !is_user_logged_in()){
?>
<div class="page-content tm-sticky-parent">
    <div <?php tutor_post_class(); ?>>
		<div class="tutor-full-width-course-body">
			<div class="container">
				<div class="row">
					<div class="col-lg-8" style="margin-top: 5rem;font-size: 20px;font-weight: bold;">
						<?php esc_html_e( 'You have to be connected to read this course', 'edumall' ); ?>
					</div>
				</div>
			</div>
		</div>
				...
				<div class="row">
					<div class="col-lg-8">
						<div class="tutor-single-course-main-content tm-sticky-column">
							...
							<?php if((!tutor_utils()->is_course_purchasable(get_the_ID()) && get_post_meta(get_the_ID(), '_tutor_is_public_course', true) == "yes") || (is_user_logged_in() && tutor_utils()->is_instructor_of_this_course(get_current_user_id(),get_the_ID())) || (is_user_logged_in() && current_user_can('administrator'))){ get_tutor_posts_attachments(); } ?>
							<?php if ( $edumall_course->is_enrolled() ) : ?>
