	...
	class Edumall_Tutor_Enqueue {
		...
		public function frontend_scripts() {
			...
			if ( Edumall_Tutor::instance()->is_dashboard() ) {
				...
				/**
				 * Tutor LMS only calls wp_enqueue_editor() when is_single()
				 * (see TUTOR\Assets, wp-content/plugins/tutor/classes/Assets.php),
				 * which never matches the Dashboard page (it's a `page`, not a
				 * `post`). Without it, TinyMCE never initializes on the profile
				 * settings "Bio" field (wp_editor() with quicktags disabled), so
				 * whatever the instructor types never syncs back to the
				 * underlying textarea on submit and the bio never actually saves.
				 */
				wp_enqueue_editor();
			}
