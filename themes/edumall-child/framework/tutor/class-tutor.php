...
if (! class_exists('Edumall_Tutor')) {
	class Edumall_Tutor
	{
		 ...
		 * @var string
		 */
		const MINIMUM_TUTOR_VERSION = '1.8.9';
		 ...
		 * @var string
		 */
		const MINIMUM_TUTOR_PRO_VERSION = '1.8.9';
		...
		{
			require_once  get_stylesheet_directory() . '/framework/tutor/course-query.php';
			require_once get_stylesheet_directory() . '/framework/tutor/course-user.php';
			require_once get_stylesheet_directory() . '/framework/tutor/class-courses.php';
			require_once get_stylesheet_directory() . '/framework/tutor/class-course.php';
			require_once get_stylesheet_directory() . '/framework/tutor/class-lesson.php';
			require_once get_stylesheet_directory() . '/framework/tutor/archive-course.php';
			require_once get_stylesheet_directory() . '/framework/tutor/single-course.php';
			require_once get_stylesheet_directory() . '/framework/tutor/single-lesson.php';
			require_once get_stylesheet_directory() . '/framework/tutor/course-builder.php';
			require_once get_stylesheet_directory() . '/framework/tutor/course-review.php';
			require_once get_stylesheet_directory() . '/framework/tutor/course-layout-switcher.php';
			require_once get_stylesheet_directory() . '/framework/tutor/shortcode.php';
			require_once get_stylesheet_directory() . '/framework/tutor/enqueue.php';
			require_once get_stylesheet_directory() . '/framework/tutor/cart.php';
			require_once get_stylesheet_directory() . '/framework/tutor/custom-css.php';
			require_once get_stylesheet_directory() . '/framework/tutor/sidebar.php';
			require_once get_stylesheet_directory() . '/framework/tutor/dashboard.php';
			require_once get_stylesheet_directory() . '/framework/tutor/profile.php';
			require_once get_stylesheet_directory() . '/framework/tutor/zoom.php';
			require_once get_stylesheet_directory() . '/framework/tutor/prerequisites.php';
			require_once get_stylesheet_directory() . '/framework/tutor/certificate.php';
			require_once get_stylesheet_directory() . '/framework/tutor/instructors.php';
			require_once get_stylesheet_directory() . '/framework/tutor/updater.php';
			require_once get_stylesheet_directory() . '/framework/tutor/class-course-translation.php';
			...
			Edumall_Tutor_Course_Translation::instance()->initialize();
		}
		...
		public function get_tax_topic()
		{
			return 'course-topic';
		}
		public function get_tax_subject()
		{
			return 'course-subject';
		}
		public function get_tax_level()
		{
			return 'course_level';
		}
		...
		/**
		 * Check if current page is topic pages
		 */
		public function is_topic()
		{
			return is_tax($this->get_tax_topic());
		}
		/**
		 * Check if current page is subject pages
		 */
		public function is_subject()
		{
			return is_tax($this->get_tax_subject());
		}
		/**
		 * Check if current page is subject pages
		 */
		...
		{
			return is_tax($this->get_tax_level());
		}
		...
		public function get_course_listing_base_url()
		{
			...
			} elseif (Edumall_Tutor::instance()->is_topic()) {
				$link = get_term_link(get_query_var('course-topic'), Edumall_Tutor::instance()->get_tax_topic());
			} elseif (Edumall_Tutor::instance()->is_subject()) {
				$link = get_term_link(get_query_var('course-subject'), Edumall_Tutor::instance()->get_tax_subject());
			} elseif (Edumall_Tutor::instance()->is_level()) {
				$link = get_term_link(get_query_var('course_level'), Edumall_Tutor::instance()->get_tax_level());
			} else {
		...
		public function get_course_listing_page_url()
		{
			...
			if (isset($_GET['filter_course-subject'])) {
				$link = add_query_arg('filter_course-subject', Edumall_Helper::data_clean(wp_unslash($_GET['filter_course-subject'])), $link);
			}
			if (isset($_GET['filter_course-theme'])) {
				$link = add_query_arg('filter_course-theme', Edumall_Helper::data_clean(wp_unslash($_GET['filter_course-theme'])), $link);
			}
			...
			if (isset($_GET['course_level'])) {
				$link = add_query_arg('filter_course_level', Edumall_Helper::data_clean(wp_unslash($_GET['filter_course_level'])), $link);
			}
		...
		{
			$terms = get_the_terms(get_the_ID(), $this->get_tax_topic());

			return $terms;
		}
		public function get_the_subjects()
		{
			$terms = get_the_terms(get_the_ID(), $this->get_tax_subject());

			return $terms;
		}
		public function get_the_levels()
		{
			$terms = get_the_terms(get_the_ID(), $this->get_tax_level());
		...
		public function get_the_level()
		{
			$terms = $this->get_the_levels();
		...
		public function get_related_courses($args)
		{
			...
			if (in_array('topics', $related_by, true)) {
				$terms = $this->get_the_topics();
				...
				if ($terms && ! is_wp_error($terms)) {
					...
					foreach ($terms as $topic) {
						if ($topic->parent === 0) {
							$term_ids[] = $topic->term_id;
						} else {
							$term_ids[] = $topic->parent;
							$term_ids[] = $topic->term_id;
						}
					}
					...
					$query_args['tax_query'][] = array(
						'taxonomy'         => $this->get_tax_topic(),
						'terms'            => $unique_term_ids,
						'include_children' => false,
					);
				}
			}
			if (in_array('subjects', $related_by, true)) {
				$terms = $this->get_the_subjects();
				...
				if ($terms && ! is_wp_error($terms)) {
					...
					foreach ($terms as $subject) {
						if ($subject->parent === 0) {
							$term_ids[] = $subject->term_id;
						} else {
							$term_ids[] = $subject->parent;
							$term_ids[] = $subject->term_id;
						}
					}
...
?>
		...
		?>
			<div class="tutor-course-categories">
				<span class="meta-label">
					...
					<?php esc_html_e('Categories', 'edumall'); ?>
				</span>
				<div class="meta-value">
					<?php
		...
		public function entry_course_topics()
		{
			$terms = get_tutor_course_topics();
		...
		?>
			<div class="tutor-course-categories">
				<span class="meta-label">
					<i class="meta-icon far fa-tag"></i>
					<?php esc_html_e('Topics', 'edumall'); ?>
				</span>
				<div class="meta-value">
					<?php
					foreach ($terms as $course_topic) {
		...
		public function entry_course_subjects()
		{
			$terms = get_tutor_course_subjects();
		...
		?>
			<div class="tutor-course-categories">
				<span class="meta-label">
					<i class="meta-icon far fa-tag"></i>
					<?php esc_html_e('Subjects', 'edumall'); ?>
				</span>
				<div class="meta-value">
					<?php
					foreach ($terms as $course_subject) {
		...
		public function entry_course_levels()
		{
			$terms = get_tutor_course_levels();
		...
		?>
			<div class="tutor-course-categories">
				...
				<div class="meta-value">
					<?php
					foreach ($terms as $course_level) {
						$level_name = $course_level->name;
						$level_link = get_term_link($course_level->term_id);
						echo "<a href='$level_link'>$level_name      </a>";
					}
					?>
				</div>
			</div>
		...
		public function course_dropdown_subjects($args = array())
		{
			global $wp_query;
			...
			$args = wp_parse_args(
				...
				array(
					...
					'show_uncategorized' => 1,
					'orderby'            => 'name',
					'selected'           => isset($wp_query->query_vars['course-subject']) ? $wp_query->query_vars['course-subject'] : '',
					'show_option_none'   => esc_html__('Select a subject', 'edumall'),
					'option_none_value'  => '',
					'value_field'        => 'slug',
					'taxonomy'           => 'course-subject',
					'name'               => 'course-subject',
					'class'              => 'dropdown-course-subject',
				)
			);
			...
			wp_dropdown_subjects($args);
		}
		public function course_dropdown_topics($args = array())
		{
			global $wp_query;
			...
			$args = wp_parse_args(
				...
				array(
					...
					'show_uncategorized' => 1,
					'orderby'            => 'name',
					'selected'           => isset($wp_query->query_vars['course-topic']) ? $wp_query->query_vars['course-topic'] : '',
					'show_option_none'   => esc_html__('Select a theme', 'edumall'),
					'option_none_value'  => '',
					'value_field'        => 'slug',
					'taxonomy'           => 'course-topic',
					'name'               => 'course-topic',
					'class'              => 'dropdown-course-topic',
				)
			);
			...
			wp_dropdown_topics($args);
		}
		public function course_dropdown_levels($args = array())
		{
			global $wp_query;
			...
			$args = wp_parse_args(
				...
				array(
					...
					'show_uncategorized' => 1,
					'orderby'            => 'name',
					'selected'           => isset($wp_query->query_vars['course_level']) ? $wp_query->query_vars['course_level'] : '',
					'show_option_none'   => esc_html__('Select a level', 'edumall'),
					'option_none_value'  => '',
					'value_field'        => 'slug',
					'taxonomy'           => 'course_level',
					'name'               => 'course_level',
					'class'              => 'dropdown-course_level',
				)
			);
		...
		public function course_dropdown_instructor($args = array())
		{
			$args = wp_parse_args(
				$args,
				array(
					...
					'hide_empty'         => 1,
					'show_uncategorized' => 1,
					'show_option_none'   => esc_html__('Select a instructor', 'edumall'),
					'option_none_value'  => '',
					'value_field'        => 'slug',
					'class'              => 'dropdown-course-topic',
				)
...
<?php
		...
		public function get_courses_ids_by_instructor($instructor_id = 0, $post_status = array('publish'))
		{
			...
			return edumall_wpml_filter_course_ids($course_ids);
		}
		...
		public function get_instructors_by_student($student_id = 0, $start = 0, $limit = 10)
		{
			global $wpdb;
			$my_instructors = [];
			...
			$my_courses = tutor_utils()->enrolled_courses_by_user($student_id, array('private', 'publish'));

			// Do nothing if this student has no publish courses.
			if (! empty($my_courses)) {
				$course_ids       = "'" . implode("','", $my_courses) . "'";
				$where_course_ids = "AND enrollment.post_parent IN({$course_ids}) ";

				$students = $wpdb->prepare(
					"SELECT DISTINCT instructor.* FROM {$wpdb->users} instructor
							INNER JOIN {$wpdb->posts} enrollment
									ON enrollment.post_author=student.ID
					WHERE 	1 =1 AND instructor.ID != %d {$where_course_ids}
							AND enrollment.post_type = %s
							AND enrollment.post_status = %s
					LIMIT 	%d, %d;
					",
					$student_id, // Skip my self from list
					'tutor_enrolled',
					'completed',
					$start,
					$limit
