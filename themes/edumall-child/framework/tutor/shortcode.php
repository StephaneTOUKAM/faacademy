...
if ( ! class_exists( 'Edumall_Tutor_Shortcode' ) ) {
	class Edumall_Tutor_Shortcode extends Edumall_Tutor {
		...
		public function get_courses() {
			...
			/**
			 * WPML compatibility: the raw SQL query above bypasses WP_Query,
			 * so WPML's language filtering never applies. Join icl_translations
			 * manually to only fetch courses of the current language.
			 */
			$sql_join_lang = '';
			...
			if ( function_exists( 'icl_object_id' ) || defined( 'ICL_SITEPRESS_VERSION' ) ) {
				...
				if ( ! empty( $current_language ) ) {
					...
					$sql_join_lang          = " INNER JOIN {$icl_translations_table} ON ( {$wpdb->posts}.ID = {$icl_translations_table}.element_id )";
					$sql_where              .= $wpdb->prepare(
						" AND {$icl_translations_table}.element_type = %s AND {$icl_translations_table}.language_code = %s",
						'post_' . $course_post_type,
						$current_language
					);
				}
			}
